...
objeto_0 <- dados %>%
  ...
  select(Ano,Mestrado,Doutorado) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_CIENCTEC$TITULO[1]
subtexto<-"Fonte: UFBA"
link <- T_ST_P_No_CIENCTEC$LINK[1]
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$Mestrado)),
                                   collapse = ' ')),']',sep = '')

data_serie1 <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$Doutorado)),
                                   collapse = ' ')),']',sep = '')
...
texto<-paste('{"title":{"text":"',titulo,
             ...
             '"graphic":[{"type":"text","left":"center","top":"bottom","z":100, "style":{"fill":"gray","text":"Obs: Ponto é separador decimal                 *Não há dados disponíveis para 2014.", "font":"8px sans-srif","fontSize":12}}],',
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_CIENCTEC$NOME_ARQUIVO_JS[1]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_CIENCTEC$NOME_ARQUIVO_JS[1],
                         sep =''))
