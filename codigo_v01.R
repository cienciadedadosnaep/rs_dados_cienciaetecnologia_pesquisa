...
nomes <- names(dados)
dados %<>% mutate(`Número de Bolsas`=round(`Número de Bolsas`/1000,1))
##  Perguntas e titulos 
...
objeto_0 <- dados %>%
  ...
  select(Ano,`Número de Bolsas`) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_CIENCTEC$TITULO[2]
subtexto<-"Fonte: FAPESB"
link <- T_ST_P_No_CIENCTEC$LINK[2]
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`Número de Bolsas`)),
                                   collapse = ' ')),']',sep = '')
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_CIENCTEC$NOME_ARQUIVO_JS[2]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_CIENCTEC$NOME_ARQUIVO_JS[2],
                         sep =''))
